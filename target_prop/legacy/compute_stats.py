...
def plot_results(stat_w, stat_angle):
    ...
    plt.fill_between(
        ...
        np.mean(stat_w, 0) + np.std(stat_w, 0),
        np.mean(stat_w, 0) - np.std(stat_w, 0),
        alpha=0.5,
        color="C0",
    )
    ...
    plt.fill_between(
        np.linspace(1, epochs, epochs),
        np.mean(stat_angle, 0) + np.std(stat_angle, 0),
        np.mean(stat_angle, 0) - np.std(stat_angle, 0),
        alpha=0.5,
        color="C2",
    )
    plt.plot(
        np.linspace(1, epochs, epochs), np.mean(stat_angle, 0), color="C2", linewidth=linewidth
    )
    plt.xlabel("Batch iterations")
...
if __name__ == "__main__":
    ...
    print(
        "Final weight distance: mean {:.2f}, std {:.2f}".format(
            np.mean(stat_w[:, -1]), np.std(stat_w[:, -1])
        )
    )
    print(
        "Final weight angle: mean {:.2f} deg, std {:.2f} deg".format(
            np.mean(stat_angle[:, -1]), np.std(stat_angle[:, -1])
        )
    )
