...
def test_sum_of_grads_backward_is_same_as_backward_of_each_grad(
    forward_net: Sequential,
    feedback_net: Sequential,
    beta: float):
    n_classes = 10
    channels = [1, 16, 32]
    beta = 0.7
    ...
    for i, ((layer_name,  layer_, layer_loss) in reversed(list(enumerate(zip(forward_net.named_children(), dtp_losses)))):
        ...
        if layer_loss.requires_grad:
            layer_loss.backward()
            dtp_grads_v0.update({f"{i}.{name}": p.grad.clone().detach() for name, p in named_trainable_parameters(layer)})
...
def main():
    ...
    # Make sure that the weights are initialized symmetrically. (same as above, but more explicit)
    assert (forward_net.conv_0.conv.weight == feedback_net.conv_0.conv.weight).all()
    assert (forward_net.conv_1.conv.weight == feedback_net.conv_1.conv.weight).all()
    assert (forward_net.linear.weight == feedback_net.linear.weight.t()).all()
    ...
    for name, backprop_grad in true_backprop_grads.items():
        ...
        l1_norm = round(backprop_grad.norm(p=1).item(), 3)
        l2_norm = round(backprop_grad.norm(p=2).item(), 3)
        print(f"\tGradient norm (backprop):\t {l1_norm=}, {l2_norm=}")

        dtp_grad = dtp_grads[name]
        l1_norm = round(dtp_grad.norm(p=1).item(), 3)
        l2_norm = round(dtp_grad.norm(p=2).item(), 3)
        ...
        if backprop_grad.dim() > 1:
            ...
            print(f"\tDistance: {distance:.3f}, angle: {angle:.3f}")

        l1_distance_between_grads = round((backprop_grad - dtp_grad).abs().sum().item(), 3)
        l2_distance_between_grads = round(F.mse_loss(backprop_grad, dtp_grad).item(), 3)
        print(f"\t{l1_distance_between_grads=}, {l2_distance_between_grads=}")
